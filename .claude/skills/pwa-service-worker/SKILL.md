...
The app is meant to be usable on stage with no signal. Firestore's IndexedDB
persistence covers the data side; the service worker covers the code side. Before
adding anything that fetches at runtime, ask what it does with no network — and make
the failure visible rather than a silent empty state.
