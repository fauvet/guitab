  ...
  async buildSvg(): Promise<void> {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        // This redraws on every keystroke that touches a chord definition, so a
        // malformed one (still being typed, or hand-edited {define:}) is logged
        // rather than shown — a notification per keystroke would be noise, not
        // information. See "Errors are never swallowed".
        try {
          this.drawChord();
        } catch (error: unknown) {
          console.error(error);
